...
def write_file(data, outfile):
    ...
    for assessment in data['assessment']:
        doc.add_heading(assessment['metadata'].get('title') or "Unknown Assessment", 0)

        # handle error from missing description
        description = assessment['metadata'].get('description') or ''
        logger.info("Writing assessment: " + (assessment['metadata'].get('title') or "Unknown Assessment") )
